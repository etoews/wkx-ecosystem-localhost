...
# The pin is a fast local command; the remote listing is a network call, so it
# gets the more generous ceiling and still stays bounded so an unreachable remote
# degrades one row instead of hanging the stream.
...
def probe_submodule(
    machine: Machine, spec: SubmoduleSpec, *, timeout: float = LS_REMOTE_TIMEOUT_S
) -> ProbeOutcome:
    ...
    unreachable remote costs a single row, never the stream.

    Args:
        machine: The seam the listing runs through.
        spec: The submodule, carrying its remote url and resolved pin.
        timeout: Per-listing wall-clock ceiling in seconds.

    Returns:
        The submodule's latest release and tags-behind count, or an unknown
        outcome when the remote could not be listed.
    """
    ...
    if latest is None:
        return ProbeOutcome(latest=None, behind=None, unknown=False)
    ...
    return ProbeOutcome(latest=latest.original, behind=behind, unknown=False)
...
def stream_submodule_probes(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(probe_submodule, machine, spec, timeout=ls_remote_timeout): spec
            for spec in specs
        }
