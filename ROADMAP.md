...
- [x] A catalogue of table ids and column keys, kebab-case like Section and Category ids (`workspace`, `claude-plugins`, `git-config-keys`, `config-off`; `working-tree`, `node-modules`), lives in Python beside the Flag Categories; `PATCH /api/view` validates against it, and a test pins the ids in `app.js` to it the way the Categories are pinned. (The Mutes table has since left this catalogue: it is a bespoke editor with an Unmute action in place of the flag rail.)
...
workflow have existed since M0, but `pre-commit` is not installed, no git hook
is wired, and the workflow runs only on a push to `main` and on a pull request.
This repo opens no pull requests of its own, so a feat branch is never checked
before it is merged. Dependabot is absent and the repo's vulnerability alerts
are off. The README carries no badges. This milestone closes each gap, makes
the hook set the one definition of the checks and has CI run that same set,
and pins what runs in CI by commit. No Collector, no Section, no route.
`pre-commit` is a project dev dependency, not a machine tool, so the board's
System Section goes on showing it missing, as it does `ruff` and `ty`.

**Deliverables**
- [ ] The pre-commit gate runs: `pre-commit` is a dev dependency (`uv add --dev pre-commit`), so the lock pins its version and Dependabot bumps it. The config names `default_install_hook_types` (`pre-commit`, `commit-msg`, `pre-push`) so one `uv run pre-commit install` wires every stage, and `uv run pre-commit run --all-files` is clean. The one third-party hook repo, `pre-commit-hooks`, is pinned at its current release; `pre-commit autoupdate` is its manual bump. README's Running section gains a short "Before you commit" subsection: the one-time wiring, what each stage checks, that `git commit -n` skips the gate while CI stays the enforcer, and that the hook set diverges from `standards/python/standards/pre-commit.md` (etoews/python-standards#1 is the planned change to the standard).
- [ ] The hook set is the one definition of the checks. Commit stage: `ruff check --fix` and `ruff format` as `local` system hooks in the project venv, the way `ty check` already runs, so `uv.lock` is the only pin and hook and CI cannot disagree; `uv lock --check` as a `local` system hook, so the lock never drifts from `pyproject.toml`; and the hygiene hooks (trailing whitespace, end of file, YAML, TOML, JSON, large files, merge markers). Message stage: a `commit-msg` hook, local and regex-only, that accepts a first line of at most 120 characters that starts with a Conventional Commits type (`feat`, `fix`, `docs`, `test`, `refactor`, `style`, `chore`, `perf`, `build`, `ci`, `revert`, with an optional scope and `!`) or with git's own `Revert "`, and refuses any other. Push stage: `pytest`, so a push carries a green suite while a red commit mid-work on a feat branch stays possible. No third-party hook repo beyond `pre-commit-hooks`.
- [ ] The CI workflow checks every branch and runs the hook set: `on: push` for all branches except `dependabot/**`, which `pull_request` already covers, with `pull_request` kept for Dependabot and outside contributors. `permissions: contents: read` at the top, a `concurrency` group per ref with `cancel-in-progress`, and `timeout-minutes` on the job. Each action is pinned to a full commit SHA with its version tag in a trailing comment (`actions/checkout` and `astral-sh/setup-uv`, each at its current major), the form Dependabot keeps current. The steps are three: `uv sync --locked`, `pre-commit run --all-files --show-diff-on-failure` in place of separate ruff and ty steps, and `pytest --cov`. The `commit-msg` stage is the one check CI cannot run, because it needs a message, not a tree.
- [ ] Dependabot watches the lock file and the workflow: `.github/dependabot.yml` with the `uv` ecosystem and the `github-actions` ecosystem, each weekly, each grouped into one PR, each with a `commit-message.prefix` of `chore` and the scope included, so the log stays Conventional. The `commit-msg` hook never sees a Dependabot commit: GitHub writes it, and hooks do not run on a rebase or a fast-forward. The `gitsubmodule` ecosystem is deliberately absent: `standards/python` is pinned to a release on purpose, and Dependabot tracks a branch head, not a tag. Four repo switches, once, by `gh api`: vulnerability alerts on, automated security fixes on, rebase merge as the only merge method, and delete branch on merge. A Dependabot PR then lands the way any branch does: CI runs on it through the `pull_request` trigger, and it is rebase-merged in GitHub or pulled and ff-merged locally; either way, no merge commit.
- [ ] Badges: one row directly under the README's title, plain Markdown images with alt text and a link each: the CI workflow status on `main` (GitHub's own badge endpoint); Python 3.14, pre-commit, and the MIT licence (static shields.io); uv, ruff, and ty (Astral's own badge endpoints, rendered by shields.io). No coverage service is added.
- [ ] README's Stack section names the gates beside the tools, and its Security posture section gains one bullet for the supply-chain gates: read-only workflow token, SHA-pinned actions, Dependabot, vulnerability alerts. CLAUDE.md gains one line under a Git heading: commit subjects follow Conventional Commits, and the `commit-msg` hook refuses any other. ARCHITECTURE.md and CONTEXT.md are untouched, because nothing in the service or its vocabulary changes.

**Hands-on artefact**
- [ ] Stage a file with a formatting fault and commit; the hook reformats it and refuses the commit. Stage the fix; the commit lands. Commit with the first line `update stuff`; the `commit-msg` hook refuses it. `feat(board): add thing` and `fix!: break thing` are accepted.
- [ ] Edit `pyproject.toml` to add a dependency without locking; `uv lock --check` refuses the commit.
- [ ] Push with a failing test; the `pre-push` stage refuses it. Fix it; the push lands.
- [ ] `uv run pre-commit run --all-files` is clean.
- [ ] Push a feat branch; the Actions tab shows a green run for that branch within a minute, with the pre-commit step listing every hook, and the badge on `main` still reads "passing".
